...
class PpromericaPipeline:

    # Database setup
    conn = sqlite3.connect('promerica.db')
    c = conn.cursor()

    def open_spider(self, spider):
        self.c.execute("""CREATE TABLE IF NOT EXISTS `promerica`
                         (date text, title text, link text, content text)""")

    def process_item(self, item, spider):
        self.c.execute("""SELECT * FROM promerica WHERE title = ? AND date = ?""",
        ...
        # Insert values
        self.c.execute("INSERT INTO promerica (date, title, link, content)"
